...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/09/15 19:18:20 by oyuhi             #+#    #+#             */
/*   Updated: 2024/09/16 10:08:46 by oyuhi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// check if c is visible charactor
// go through charset and compare one by one
...
// function generating result(char**) memories
// in the while loop count how many words in str
...
// insert the founded word into result[i]
// get start and end of word, malloc space for the word
// copy a charactor one by one
char	*ft_malloc_word(char *str, int start, int end)
{
	char	*word;
	int		i;
...
// i = index of str
// index = index of result (each word)
// start = store beggining of word
// 1, malloc result
// 2, iterate str and find a first charactor of a word
// 3, count the word length
// 4, insert result the word
// 5, insert 0 at the last index and return
	...
	while (str[i])
	{
		if (!ft_is_separator(str[i], charset))
		{
			...
			while (str[i] && !ft_is_separator(str[i], charset))
				i++;
			result[index] = ft_malloc_word(str, start, i);
			if (!result[index++])
				return (NULL);
		}
		else
